...
# ---------------------------------------------------------------- boletim no zap
def _views_do_post(r):
    """'Views' como o dono vê no app: plays (Reels) ou reach (card) — o maior dos dois."""
    return max(r["plays"] or 0, r["reach"] or 0)


def run(enviar=True, dias=7):
    """🏆 BOLETIM DA SEMANA no zap (20/ago — 'dos números de views é pra você aprender'):
    campeões e fracassos dos últimos 7 dias + aula por categoria, direto do post_insights
    que o insights.py já coleta todo dia (zero chamada Graph nova). Segunda 07h45."""
    conn = _db()
    try:
        rows = conn.execute(
            """SELECT n.id, n.title, n.title_own, n.category, n.city, n.ig_permalink,
                      p.reach, p.saved, p.shares, p.comments, p.plays
               FROM post_insights p JOIN news n ON n.id = p.news_id
        ...
        conn.close()
        return {"ok": False, "motivo": f"sem post_insights ({e})"}
    conn.close()
    if not rows:
        return {"ok": False, "motivo": "nenhum post medido na janela"}

    posts = sorted(rows, key=_views_do_post, reverse=True)
    top = posts[:5]
    flop = [p for p in posts[-3:] if p not in top]
    cats = {}
    for r in posts:
        cats.setdefault((r["category"] or "geral").lower(), []).append(_views_do_post(r))
    ...
    for i, r in enumerate(top, 1):
        L.append(f"{i}º {_fmt(_views_do_post(r))} — {(r['title_own'] or r['title'] or '')[:70]} "
                 f"({_cidade(r)})")
    if flop:
        L += ["", "*💤 FICARAM PRA TRÁS:*"]
        L += [f"• {_fmt(_views_do_post(r))} — {(r['title_own'] or r['title'] or '')[:55]}"
              for r in flop]
    L += ["", "*📚 AULA (média por categoria):*"]
    L += [f"• {c}: {_fmt(m)} ({n} posts)" for c, m, n in aula[:6]]
    if top and top[0]["ig_permalink"]:
        L += ["", f"👉 Campeão: {top[0]['ig_permalink']}"]
    rel = "\n".join(L)
    ...
    if enviar:
        try:
            ...
            return {"ok": False, "motivo": f"zap falhou: {e}"}
    return {"ok": True, "medidos": len(posts),
            "campeao": (top[0]["title_own"] or top[0]["title"] or "")[:60]}
